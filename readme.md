...
Inazuma (“Ina”) is an **Experience Learning Model (ELM)**: an emergent AGI
runtime that learns through retained, provenance-aware experience rather than
operating as a Large Language Model (LLM). Her architecture focuses on
**symbolic cognition, emotional state modeling, and self‑programming**.
...
## TL;DR
- **ELM, not LLM.** Experience Learning Model is the project term for Ina's
  experience-led architecture; it is a description of how she learns, not a
  claim that every component is novel or that language is unimportant.
